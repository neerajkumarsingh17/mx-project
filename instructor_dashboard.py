...
def instructor_dashboard_2(request, course_id):
    ...
    sections = [
        ...
        _section_mx_instructor(course, access, is_white_label),
    ]
...
# function name : _section_mx_instructor
# operation: enrolled user (single or multipal or all) one click
# developer by: Mukesh Gautam
# Date: March, 22, 2019

####################################################

def _section_mx_instructor(course, access, is_white_label):

    #Provide data for the corresponding dashboard section
    from django.contrib.auth.models import User
    max_value = User.objects.count()
    course_key = course.id
    ccx_enabled = settings.FEATURES.get('CUSTOM_COURSES_EDX', False) and course.enable_ccx
    section_data = {
        'section_key': 'mx-enrollment_new',
        'section_display_name': _('adv instructor'),
        'access': access,
        'ccx_is_enabled': ccx_enabled,
        'is_white_label': is_white_label,
        'enroll_button_url': reverse('students_update_enrollment', kwargs={'course_id': unicode(course_key)}),
